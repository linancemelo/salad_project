<template>
  <div class="project-slider">
    <div class="cover"></div>
    <div
      id="carouselExampleControls"
      class="carousel slide"
      data-bs-ride="carousel"
      ref="myCarousel"
    >
      <div class="carousel-inner">
        <div class="carousel-item active">
          <div class="index-container">
            <div class="current-index">
              <div class="current">01</div>
              <div class="line"></div>
              <div class="total">04</div>
            </div>
          </div>
          <div class="img">
            <img src="https://ppt.cc/fFtsTx@.png" class="img-fluid" alt="..." />
          </div>
          <div class="content">
            <div class="top">HIGHLIGHTED SHOWCASE</div>
            <div class="title-container">
              <h4>LP CLUB MOBILE APP<span class="dot"></span></h4>
              <span class="line"></span>
            </div>
            <div class="description">
              Cras quis nulla commodo, aliquam lectus sed, blandit augue. Cras
              ullamcorper bibendum bibendum. Duis tincidunt urna non pretium
              porta. Nam condimentum vitae ligula vel ornare. Phasellus at
              semper turpis. Nunc eu tellus tortor. Etiam at condimentum nisl,
              vitae sagittis orci. Donec id dignissim nunc. Donec elit ante,
              eleifend a dolor et, venenatis facilisis dolor. In feugiat orci
              odio, sed lacinia sem elementum quis. Aliquam consectetur, eros et
              vulputate euismod, nunc leo tempor lacus, ac rhoncus neque eros
              nec lacus. Cras lobortis molestie faucibus.
            </div>
          </div>
        </div>
        <div class="carousel-item">
          <div class="index-container">
            <div class="current-index">
              <div class="current">02</div>
              <div class="line"></div>
              <div class="total">04</div>
            </div>
          </div>
          <div class="img">
            <img src="https://ppt.cc/fFtsTx@.png" class="img-fluid" alt="..." />
          </div>
          <div class="content">
            <div class="top">HIGHLIGHTED SHOWCASE</div>
            <div class="title-container">
              <h4>LP CLUB MOBILE APP<span class="dot"></span></h4>
              <span class="line"></span>
            </div>
            <div class="description">
              Cras quis nulla commodo, aliquam lectus sed, blandit augue. Cras
              ullamcorper bibendum bibendum. Duis tincidunt urna non pretium
              porta. Nam condimentum vitae ligula vel ornare. Phasellus at
              semper turpis. Nunc eu tellus tortor. Etiam at condimentum nisl,
              vitae sagittis orci. Donec id dignissim nunc. Donec elit ante,
              eleifend a dolor et, venenatis facilisis dolor. In feugiat orci
              odio, sed lacinia sem elementum quis. Aliquam consectetur, eros et
              vulputate euismod, nunc leo tempor lacus, ac rhoncus neque eros
              nec lacus. Cras lobortis molestie faucibus.
            </div>
          </div>
        </div>
        <div class="carousel-item">
          <div class="index-container">
            <div class="current-index">
              <div class="current">03</div>
              <div class="line"></div>
              <div class="total">04</div>
            </div>
          </div>
          <div class="img">
            <img src="https://ppt.cc/fFtsTx@.png" class="img-fluid" alt="..." />
          </div>
          <div class="content">
            <div class="top">HIGHLIGHTED SHOWCASE</div>
            <div class="title-container">
              <h4>LP CLUB MOBILE APP<span class="dot"></span></h4>
              <span class="line"></span>
            </div>
            <div class="description">
              Cras quis nulla commodo, aliquam lectus sed, blandit augue. Cras
              ullamcorper bibendum bibendum. Duis tincidunt urna non pretium
              porta. Nam condimentum vitae ligula vel ornare. Phasellus at
              semper turpis. Nunc eu tellus tortor. Etiam at condimentum nisl,
              vitae sagittis orci. Donec id dignissim nunc. Donec elit ante,
              eleifend a dolor et, venenatis facilisis dolor. In feugiat orci
              odio, sed lacinia sem elementum quis. Aliquam consectetur, eros et
              vulputate euismod, nunc leo tempor lacus, ac rhoncus neque eros
              nec lacus. Cras lobortis molestie faucibus.
            </div>
          </div>
        </div>
        <div class="carousel-item">
          <div class="index-container">
            <div class="current-index">
              <div class="current">04</div>
              <div class="line"></div>
              <div class="total">04</div>
            </div>
          </div>
          <div class="img">
            <img src="https://ppt.cc/fFtsTx@.png" class="img-fluid" alt="..." />
          </div>
          <div class="content">
            <div class="top">HIGHLIGHTED SHOWCASE</div>
            <div class="title-container">
              <h4>LP CLUB MOBILE APP<span class="dot"></span></h4>
              <span class="line"></span>
            </div>
            <div class="description">
              Cras quis nulla commodo, aliquam lectus sed, blandit augue. Cras
              ullamcorper bibendum bibendum. Duis tincidunt urna non pretium
              porta. Nam condimentum vitae ligula vel ornare. Phasellus at
              semper turpis. Nunc eu tellus tortor. Etiam at condimentum nisl,
              vitae sagittis orci. Donec id dignissim nunc. Donec elit ante,
              eleifend a dolor et, venenatis facilisis dolor. In feugiat orci
              odio, sed lacinia sem elementum quis. Aliquam consectetur, eros et
              vulputate euismod, nunc leo tempor lacus, ac rhoncus neque eros
              nec lacus. Cras lobortis molestie faucibus.
            </div>
          </div>
        </div>
      </div>
      <button
        class="carousel-control-prev"
        type="button"
        data-bs-target="#carouselExampleControls"
        data-bs-slide="prev"
      >
        <span class="carousel-control-prev-icon" aria-hidden="true">
          <i class="bi bi-chevron-left"></i>
        </span>
        <span class="visually-hidden">Previous</span>
      </button>
      <button
        class="carousel-control-next"
        type="button"
        data-bs-target="#carouselExampleControls"
        data-bs-slide="next"
      >
        <span class="carousel-control-next-icon" aria-hidden="true">
          <i class="bi bi-chevron-right"></i>
        </span>
        <span class="visually-hidden">Next</span>
      </button>
    </div>
  </div>
</template>

<script>
import Carousel from 'bootstrap/js/dist/carousel'
export default {
  mounted () {
    const carousel = new Carousel(this.$refs.myCarousel)
    carousel.cycle()
  }
}
</script>

<style scoped lang="scss" src="@/assets/scss/ProjectSlider.scss"></style>
